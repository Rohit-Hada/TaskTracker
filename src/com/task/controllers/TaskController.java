...
/*	String taskStatusSave(@RequestParam CommonsMultipartFile file,Model m,HttpSession sess, Fileupload uploadFile, HttpServletRequest req, Remarks r, Tasks t,Task_History th) throws IOException*/
	String taskStatusSave(Model m,HttpSession sess, Fileupload uploadFile, HttpServletRequest req, Remarks r, Tasks t,Task_History th) throws IOException
	{  		
			String url="";
			HttpSession session = req.getSession(false);
		    String currentUserName = (String)session.getAttribute("currentUserName");
		    ...
		    {  
		     ...
		     if(req.getParameter("remark")!= null && !req.getParameter("remark").isEmpty())
				...
				/*
				 * if( !file.getOriginalFilename().isEmpty()) { updates =
				 * updates+" File upload Remark no."+r.getId(); }
				 */
				...
				/*
				 * if(!file.getOriginalFilename().isEmpty()) { DiskFileItemFactory factory = new
				 * DiskFileItemFactory(); factory.setSizeThreshold(THRESHOLD_SIZE);
				 * factory.setRepository(new File(System.getProperty("java.io.tmpdir")));
				 * ServletFileUpload upload = new ServletFileUpload(factory); ServletContext
				 * context = sess.getServletContext(); String uploadPath =
				 * context.getRealPath(UPLOAD_DIRECTORY); byte[] bytes = file.getBytes(); String
				 * filename = file.getOriginalFilename().split("\\.")[0]; String extensen =
				 * file.getOriginalFilename().split("[.]")[1]; BufferedOutputStream stream =new
				 * BufferedOutputStream(new FileOutputStream(new File(uploadPath +
				 * File.separator + filename+timemili+"."+extensen))); stream.write(bytes);
				 * stream.flush(); stream.close();
				 * uploadFile.setName(filename+timemili+"."+extensen);
				 * uploadFile.setData(bytes); uploadFile.setRemark_id(r.getId());
				 * taskService.saveFile(uploadFile); }
				 */
